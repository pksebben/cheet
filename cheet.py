...
Options:
 ...
 Cheet.py creates a keybinding and saves it in cheet-keybinds.json.
 ...
 Cheet creates a file 'cheet-keybinds.json' that represents a dict of
...
def loop():
    kb_list = json.load(open("cheet-keybinds.json", "r"))
...
def save_kb_list(kb_list):
    json.dump(kb_list, open("cheet-keybinds.json", "w"))
    save_and_compile()
...
def main():
    ...
    try:
        keybind_list = json.load(open("cheet-keybinds.json", "r"))
    except FileNotFoundError:
        print("No cheet-keybinds.json found.  Initializing fresh cheet-keybinds.json")
        keybind_list = []
    if args['KEYBIND']:
    ...
    json.dump(keybind_list, open("cheet-keybinds.json", "w"))
    cheet_compile.compile_cheetsheet()
    cheet_compile.save_cheetsheet(cheet_compile.compile_cheetsheet())
    cheet_compile.chrome_headless_render()
    return 1
